...
class RGB2DensePose:
    ...
    def folder2densepose(self, input_folder):
        ...
        os.system("python apply_net.py dump configs/densepose_rcnn_R_101_FPN_s1x.yaml https://dl.fbaipublicfiles.com/densepose/densepose_rcnn_R_101_FPN_s1x/165712084/model_final_c6ab63.pkl " + input_folder + " --output " + densepose_pkl_directory + "/a.pkl" + " -v")
       # files = os.listdir(input_folder)
#	print(files)
        path = densepose_pkl_directory + "/a.pkl"
        self.img2densepose(path,input_folder)
    ...
    def img2densepose(self, output_pkl,input_folder):
        ...
        for input_image in os.listdir(input_folder):
            if input_image.endswith('.jpg') or input_image.endswith('.png'):
                input_image = os.path.join(input_folder, input_image)
                output_image = input_image.replace('.jpg', '_densepose.png').replace('.png', '_densepose.png')
               # self.img2densepose(input_image, output_pkl, output_image)
                ...
                i       = data[i]['pred_densepose'][0].labels.cpu().numpy()
                uv      = data[i]['pred_densepose'][0].uv.cpu().numpy()
